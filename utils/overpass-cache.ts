...
class OverpassCache {
  ...
  private getCacheKey(lat: number, lon: number, tags: string[]): string {
    ...
    return `${roundedLat},${roundedLon}:${sortedTags}`;
  }

  // Get cached result if available and not expired
  get(lat: number, lon: number, tags: string[]): unknown | null {
    const key = this.getCacheKey(lat, lon, tags);
  ...
  set(lat: number, lon: number, tags: string[], data: unknown, ttl?: number): void {
    const key = this.getCacheKey(lat, lon, tags);
    this.cache.set(key, {
      data,
      timestamp: Date.now(),
